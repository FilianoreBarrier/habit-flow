...
async def change_user_password(
    ...
    user_service = UserService(db)
    return await user_service.change_password(current_user.user_id, change_data.old_password, change_data.new_password)

@router.post('/me/deactivate', response_model=UserResponse, status_code=status.HTTP_200_OK)
async def deactivate_user(db: AsyncSession = Depends(get_db),current_user:UserResponse = Depends(get_current_user)):
    user_service = UserService(db)
    return await user_service.deactivate_user(current_user.user_id)
