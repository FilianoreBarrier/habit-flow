...
class HabitLogService:
    ...
    async def create_log(self, log_data: HabitLogCreate, habit_id: int, user_id: int) -> HabitLogResponse:
        ...
        log = await self.habit_log_repository.create(log_data, user_id, habit_id)

        return HabitLogResponse.model_validate(log)

    async def get_logs_by_habit(self, habit_id: int, user_id: int) -> list[HabitLogResponse]:
        if not await self.user_repository.get_by_id(user_id):
            raise_user_not_found(user_id)
        ...
        logs = await self.habit_log_repository.get_by_habit_id(habit_id)
        return [HabitLogResponse.model_validate(log) for log in logs]

    async def get_logs_by_user(self, user_id: int) -> list[HabitLogResponse]:

        if not await self.user_repository.get_by_id(user_id):
            raise_user_not_found(user_id)

        logs = await self.habit_log_repository.get_by_user_id(user_id)
        return [HabitLogResponse.model_validate(log) for log in logs]


    async def get_log_by_id(self, log_id: int, user_id: int) -> HabitLogResponse:
        log = await self.habit_log_repository.get_by_id(log_id)
        if not log or log.user_id != user_id:# type: ignore[attr-async defined]
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def update_log(self, log_id: int, log_update: HabitLogUpdate, user_id: int) -> HabitLogResponse:
        ...
        if log.user_id != user_id: # type: ignore[attr-async defined]
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail='You can only update your own habit logs! '
            )

        updated_log = await self.habit_log_repository.update(log_id, log_update)
    ...
    async def delete_log(self, log_id: int, user_id: int) -> bool:
        ...
        await self.habit_log_repository.delete(log_id)
        return True
